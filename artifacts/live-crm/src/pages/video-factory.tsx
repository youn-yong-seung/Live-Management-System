    ...
    <Link href={`/video-factory/${plan.id}`}>
      <div className="group glass-card p-6 cursor-pointer hover:bg-[#f7f8fa] hover:border-white/[0.12] transition-all duration-200">
        <div className="flex items-start gap-5">
          ...
          <ChevronRight className="w-5 h-5 text-white/20 group-hover:text-[#484d57] group-hover:translate-x-1 transition-all flex-shrink-0 mt-2" />
        </div>
      </div>
    </Link>
  );
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-3 gap-3">
        <div className="glass-card px-5 py-4">
          ...
          <p className="text-2xl font-bold text-white">{PLANS.length}<span className="text-sm text-[#a0a4ab] font-normal ml-1">개</span></p>
        </div>
        <div className="glass-card px-5 py-4">
          <p className="text-[10px] text-[#8b8f98] uppercase tracking-wider mb-1">총 버전</p>
          <p className="text-2xl font-bold text-white">{PLANS.reduce((s,p)=>s+p.versions.length,0)}<span className="text-sm text-[#a0a4ab] font-normal ml-1">개</span></p>
        </div>
        <div className="glass-card px-5 py-4">
          <p className="text-[10px] text-[#8b8f98] uppercase tracking-wider mb-1">평균 배율</p>
          <p className="text-2xl font-bold text-white">{(PLANS.reduce((s,p)=>s+getLatestVersion(p).anchor.ratio,0)/PLANS.length).toFixed(1)}<span className="text-sm text-[#a0a4ab] font-normal ml-1">x</span></p>
        </div>
      </div>
      ...
      <div className="relative">
        ...
        <Input
          ...
          className="pl-10 bg-white/[0.03] border-[#e5e7eb] text-white placeholder:text-[#a0a4ab]"
        />
        {searchQuery && (
