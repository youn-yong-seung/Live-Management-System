...
function VideoRefRow({ video, anchor = false }: { video: VideoRef; anchor?: boolean }) {
  ...
  return (
    <a
      ...
      className={`group flex gap-4 p-4 rounded-md transition-all duration-200 ${video.url ? "hover:bg-white/[0.03] cursor-pointer" : "opacity-60"} ${anchor ? "border border-[#e5e7eb] bg-[#f7f8fa]" : "border border-white/[0.05]"}`}
    >
      ...
      <div className="flex-1 min-w-0">
        <p className="text-sm text-[#111318] line-clamp-2 leading-snug font-medium">{video.title}</p>
        <p className="text-xs text-[#8b8f98] mt-1">{video.channel}</p>
        <div className="flex items-center gap-3 mt-2 text-xs text-[#a0a4ab]">
...
function Section({ title, children, sub }: { title: string; sub?: string; children: React.ReactNode }) {
  return (
    <section className="space-y-3">
      <div>
        <h3 className="text-xs font-semibold text-[#8b8f98] uppercase tracking-wider">{title}</h3>
        {sub && <p className="text-[10px] text-[#a0a4ab] mt-0.5">{sub}</p>}
      </div>
      <div>{children}</div>
    </section>
  );
    ...
    <div className="space-y-10">
      ...
      <Section title="1. 영상 개요">
        <div className="grid sm:grid-cols-2 gap-4">
          ...
          <div className="glass-card p-5">
            ...
            <p className="text-sm text-[#111318]">{v.subtitle}</p>
          </div>
          <div className="glass-card p-5">
            <p className="text-[10px] text-[#8b8f98] uppercase mb-1.5 flex items-center gap-1.5"><Target className="w-3 h-3" />타겟</p>
            <p className="text-sm text-[#111318]">{v.target}</p>
          </div>
          <div className="glass-card p-5">
            <p className="text-[10px] text-[#8b8f98] uppercase mb-1.5 flex items-center gap-1.5"><Clock className="w-3 h-3" />목표 길이</p>
            <p className="text-sm text-[#111318]">{v.targetLength}</p>
          </div>
          <div className="glass-card p-5 sm:col-span-2">
            <p className="text-[10px] text-[#8b8f98] uppercase mb-1.5 flex items-center gap-1.5"><Lightbulb className="w-3 h-3" />핵심 메시지</p>
            <p className="text-sm text-[#111318] leading-relaxed">{v.coreMessage}</p>
          </div>
          <div className="glass-card p-5 sm:col-span-2">
            <p className="text-[10px] text-[#8b8f98] uppercase mb-1.5">형식</p>
            <p className="text-sm text-[#111318]">{v.format}</p>
          </div>
        </div>
      </Section>
      ...
      <Section title="2. 벤치마킹 분석">
        <div className="space-y-4">
          {/* 앵커 영상 */}
          <div className="glass-card p-5">
            <p className="text-[10px] text-[#8b8f98] uppercase mb-3 flex items-center gap-1.5"><Anchor className="w-3 h-3" />앵커 벤치마킹 영상</p>
          ...
          <div className="grid sm:grid-cols-3 gap-3">
            <div className="glass-card p-5">
              <p className="text-[10px] text-[#8b8f98] uppercase mb-1.5">채널 규모</p>
              <p className="text-sm text-[#111318]">{toKr(v.anchor.subs)} 구독자</p>
            </div>
            <div className="glass-card p-5">
              <p className="text-[10px] text-[#8b8f98] uppercase mb-1.5">성과</p>
              <p className="text-sm text-[#111318]">{toKr(v.anchor.views)}뷰 / {v.anchor.ratio.toFixed(1)}x</p>
            </div>
            <div className="glass-card p-5">
              <p className="text-[10px] text-[#8b8f98] uppercase mb-1.5">특징</p>
              <p className="text-xs text-[#484d57] leading-relaxed">{v.rationale.structural}</p>
            </div>
          </div>

          {/* 시사점 */}
          <div className="glass-card p-5">
            <p className="text-[10px] text-[#8b8f98] uppercase mb-3">벤치마킹 시사점</p>
            <ul className="space-y-2">
              {v.insights.map((insight, i) => (
                <li key={i} className="flex gap-2 text-sm text-[#484d57]">
                  <span className="text-[#a0a4ab] flex-shrink-0">•</span>
                  <span>{insight}</span>
                </li>
              ))}
            </ul>
      ...
      <Section title="3. 썸네일 & 제목">
        <div className="grid sm:grid-cols-3 gap-4">
          ...
          <div className="glass-card p-5 sm:col-span-2 flex flex-col justify-center">
            <p className="text-[10px] text-[#8b8f98] uppercase mb-2">영상 제목 (SEO)</p>
            <p className="text-sm text-[#111318] leading-relaxed">{v.videoTitle}</p>
          </div>
        </div>
      </Section>
      ...
      <Section title="4. 인트로 (30초 룰)">
        <div className="glass-card p-5 space-y-4">
          <div>
            <p className="text-[10px] text-[#8b8f98] uppercase mb-1.5">위기 환기</p>
            <p className="text-sm text-[#111318] leading-relaxed">{v.intro.crisis}</p>
          </div>
          <div className="border-t border-[#eef0f3] pt-4">
            <p className="text-[10px] text-[#8b8f98] uppercase mb-1.5">공감</p>
            <p className="text-sm text-[#111318] leading-relaxed">{v.intro.empathy}</p>
          </div>
          <div className="border-t border-[#eef0f3] pt-4">
            <p className="text-[10px] text-[#8b8f98] uppercase mb-1.5">약속 + 행동 명령</p>
            <p className="text-sm text-[#111318] leading-relaxed">{v.intro.promise}</p>
          </div>
          <button
            onClick={() => copyText(`${v.intro.crisis}\n${v.intro.empathy}\n${v.intro.promise}`, "intro")}
            className="text-xs text-[#8b8f98] hover:text-[#484d57] flex items-center gap-1.5"
          >
      ...
      <Section title="5. 본문 구조">
        <div className="glass-card p-5 space-y-6">
          {/* 기본 세팅 */}
          <div>
            <p className="text-[10px] text-[#8b8f98] uppercase mb-2">기본 세팅</p>
            <ul className="space-y-1.5">
              {v.body.setup.map((s, i) => (
                <li key={i} className="text-sm text-[#484d57] flex gap-2">
                  <span className="text-[#a0a4ab]">{i + 1}.</span>
                  <span>{s}</span>
                </li>
              ))}
            </ul>
          ...
          <div className="border-t border-[#eef0f3] pt-6">
            <p className="text-[10px] text-[#8b8f98] uppercase mb-3">
              ...
              {v.body.practices.map((p, i) => (
                <div key={i} className="space-y-2">
                  <div className="flex gap-3">
                    <span className="w-6 h-6 rounded-full bg-[#eef0f3] text-[#484d57] text-xs flex items-center justify-center font-semibold flex-shrink-0">
                      {i + 1}
                    </span>
                    <p className="text-sm text-[#111318] leading-relaxed pt-0.5">{p.title}</p>
                  </div>
                  {p.prompt && (
                    <div className="ml-9 p-3 rounded bg-white/[0.03] border border-[#eef0f3] text-xs text-[#484d57] font-mono leading-relaxed">
                      {p.prompt}
                    </div>
                  )}
                  {p.highlight && (
                    <p className="ml-9 text-xs text-[#8b8f98]">⭐ {p.highlight}</p>
                  )}
                </div>
              ))}
            </div>
          ...
          <div className="border-t border-[#eef0f3] pt-6">
            <p className="text-[10px] text-[#8b8f98] uppercase mb-2">중간 CTA</p>
            <p className="text-sm text-[#111318] leading-relaxed">{v.body.midCta}</p>
          </div>
        </div>
      </Section>
      ...
      <Section title="6. 엔딩 CTA (3중 구조)">
        <div className="glass-card p-5 space-y-4">
          <div>
            <p className="text-[10px] text-[#8b8f98] uppercase mb-1.5">리드 수집</p>
            <p className="text-sm text-[#111318] leading-relaxed">{v.endingCta.leadMagnet}</p>
          </div>
          <div className="border-t border-[#eef0f3] pt-4">
            <p className="text-[10px] text-[#8b8f98] uppercase mb-1.5">구독 유도</p>
            <p className="text-sm text-[#111318] leading-relaxed">{v.endingCta.subscribe}</p>
          </div>
          <div className="border-t border-[#eef0f3] pt-4">
            <p className="text-[10px] text-[#8b8f98] uppercase mb-1.5">댓글 유도</p>
            <p className="text-sm text-[#111318] leading-relaxed">{v.endingCta.comment}</p>
          </div>
          {v.outro && (
            <div className="border-t border-[#eef0f3] pt-4">
              <p className="text-[10px] text-[#8b8f98] uppercase mb-1.5">아웃트로 멘트</p>
              <p className="text-sm text-[#111318] italic leading-relaxed">"{v.outro}"</p>
            </div>
          )}
        </div>
      </Section>
      ...
      <div className="pt-4">
        <button
          ...
          className="w-full py-3.5 rounded-md bg-[#eef0f3] hover:bg-[#e5e7eb] text-[#111318] text-sm font-medium border border-white/[0.08] transition-colors flex items-center justify-center gap-2"
        >
...
export default function VideoFactoryDetail() {
    ...
    return (
      <div className="space-y-4">
        ...
        <div className="glass-card p-12 text-center">
          <p className="text-[#8b8f98]">기획안을 찾을 수 없습니다</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="space-y-4">
        <Link href="/video-factory">
          <button className="text-xs text-[#8b8f98] hover:text-[#111318] flex items-center gap-1.5 transition-colors">
        ...
        <div className="flex items-start justify-between gap-4 flex-wrap">
          ...
          {plan.versions.length > 1 && (
            <div className="flex items-center gap-2">
              <History className="w-4 h-4 text-[#8b8f98]" />
        ...
        {currentVersion.changeNote && (
          <div className="glass-card p-4 border-l-2 border-white/30">
            <p className="text-[10px] text-[#8b8f98] uppercase mb-1 flex items-center gap-1.5">
              <MessageSquare className="w-3 h-3" /> {currentVersion.versionLabel} 변경 사유 ({currentVersion.createdAt})
            </p>
            <p className="text-xs text-[#484d57] leading-relaxed">{currentVersion.changeNote}</p>
          </div>
        )}
      </div>
